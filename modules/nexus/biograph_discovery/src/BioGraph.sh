...
main() {
    ...
    echo "Value of biograph: '$biograph_file'"
    ...
    dx download "$biograph_file" biograph.bg.tgz && tar xvf biograph.bg.tgz &
    mkdir reference
    ...
    bg_name=*.bg
    output_prefix=$(basename ${bg_name} .bg)
    ref_name=reference/$(ls reference/) 
    echo "biograph: " ${bg_name}
    echo "reference: " ${ref_name}
    
    biograph full_pipeline --resume discovery -b ${bg_name} -r ${ref_name} -m model.ml --reads ${read_name} "${cust_args[@]}"
