...
#from biograph.utils import setup_logging, get_opener
...
#    setup_logging()
    if args.clsf & 2 and args.grm is None:
        logging.error("Expecting --grm dataframe for this classifier run. Exiting")
        exit(1)
...
def run_gt(classifier, data, features=None):
    """
    Run gt classification - drops then replaces "GT" column
    """
    #to_classify = data.dropna(subset=features)
    data["GT_pred"] = classifier.predict(data[features])
    data[['prob_ref', 'prob_het', 'prob_hom']] = classifier.predict_proba(data[features])
    data = data.drop(["GT", "GQ", "PL_het", "PL_hom", "PL_ref"], axis=1, errors='ignore')
    data["GT"] = data["GT_pred"].copy()
    ## replacing '0' probabilites with 1/n_estimator  | n_estimators =500
    data['prob_ref'] = data['prob_ref'].apply(lambda x: max(x, 0.002))
    data['prob_het'] = data['prob_het'].apply(lambda x: max(x, 0.002))
    data['prob_hom'] = data['prob_hom'].apply(lambda x: max(x, 0.002))
    data['PL_ref'] = -10*np.log10(data['prob_ref'])
    data['PL_het'] = -10*np.log10(data['prob_het'])
    data['PL_hom'] = -10*np.log10(data['prob_hom'])
    data['GQ'] = get_gq(data)
    return (data)
...
# @profile
def edit_vcf(chunk):# pylint: disable=too-many-statements
    """
    ...
    for entry in region:
        ...
        if clsf & 2:
            ...
            if score < filter_thresh:
                stats["filt_cnt"] += 1
                continue
            qual = str(int(score * 100) if not np.isnan(score) else 0)
            if score < lowqual:
                filt = "lowq"
                stats["lowq_cnt"] += 1
            else:
...
def main(args):
    ...
    data_sv = norm_rl(data_sv) ## Using norm for both QUAL & GT
    ...
    # Add GT
    if args.clsf & 1:
        logging.info("Classifying GT")
        data_sv = run_gt(model["model_gt"], data_sv, model["features_gt"])
        if args.clsf == 1:
            data_sv["tp"] = 1
    ...
    if args.clsf & 2:
        ...
        logging.info("Classifying SV, SNPs & INDELS")
    ...
    if args.clsf & 1:
        ...
        logging.info(f'Homozygous {stats["hom_cnt"]}; Heterozygous {stats["het_cnt"]}; Ratio {ratio:.2f}')

    if args.clsf & 2:
        logging.info(f'None {stats["none_cnt"]}; Removed {stats["filt_cnt"]}; LowQ {stats["lowq_cnt"]}; PASS {stats["pass_cnt"]}')
